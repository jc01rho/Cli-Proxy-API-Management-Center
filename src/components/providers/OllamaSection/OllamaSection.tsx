    ...
    configs.forEach((config, index) => {
      ...
      cache.set(
        ...
        statusBarDataFromRecentRequests(
          getProviderRecentBuckets(usageByProvider, 'ollama', config.apiKey, config.baseUrl)
        )
      );
    });
    return cache;
    ...
    <>
      ...
      >
        <ProviderList<ProviderKeyConfig>
          ...
          emptyTitle={t('ai_providers.ollama_empty_title')}
          emptyDescription={t('ai_providers.ollama_empty_desc')}
          ...
          renderContent={(item, index) => {
            ...
            const statusData = statusBarCache.get(getProviderConfigKey(item, index)) || statusBarDataFromRecentRequests([]);

            return (
              <Fragment>
                <div className="item-title">{t('ai_providers.ollama_item_title')}</div>
                ...
                {item.models?.length ? (
                  <div className={styles.modelTagList}>
                    <span className={styles.modelCountLabel}>
                      {t('ai_providers.ollama_models_count')}: {item.models.length}
                    </span>
                ...
                {excludedModels.length ? (
                  <div className={styles.excludedModelsSection}>
                    <span className={styles.excludedModelsLabel}>
                      {t('ai_providers.excluded_models_label')}
                    </span>
                    <div className={styles.excludedModelsList}>
                      {excludedModels.map((model) => (
                        <span key={model} className={styles.excludedModelTag}>
                          {model}
                        </span>
                      ))}
                    </div>
