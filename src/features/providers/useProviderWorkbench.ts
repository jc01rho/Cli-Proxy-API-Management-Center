...
import type { Config, GeminiKeyConfig, ModelAlias, OpenAIProviderConfig, ProviderKeyConfig } from '@/types';
import {
  apiKeyFunToResource,
  claudeToResource,
  codexToResource,
...
export const buildProviderGroups = (config: Config): ProviderGroup[] => {
  // Temporarily hidden sponsors keep their provider configs editable: their protocol groups own the display instead (see sponsorDefinitions.ts).
...
export function useProviderWorkbench(): UseProviderWorkbenchResult {
    ...
    async (brand: ProviderBrand, input: ProviderEntryFormInput) => {
      ...
      try {
        if (brand === 'gemini') {
          ...
          await providersApi.createFreebuffConfig(
            buildProviderKeyConfig('freebuff', input) as ProviderKeyConfig
          );
  ...
  const updateProvider = useCallback(
    async (resource: ProviderResource, input: ProviderEntryFormInput) => {
      ...
      try {
        ...
        } else if (brand === 'freebuff' && selector.brand === 'freebuff') {
          const existing = resource.raw as ProviderKeyConfig;
          await providersApi.updateFreebuffConfigByKey(
            selector.apiKey,
            selector.baseUrl,
            buildProviderKeyConfig('freebuff', input, existing) as ProviderKeyConfig
          );
        } else if (brand === 'mistral' && selector.brand === 'mistral') {
          const existing = resource.raw as ProviderKeyConfig;
          await providersApi.updateMistralConfigByKey(
            selector.apiKey,
            selector.baseUrl,
  ...
  const deleteProvider = useCallback(
    ...
    [clearCache, config, refetch, updateConfigValue]
  );
      ...
      try {
        ...
        if (brand === 'gemini' && selector.brand === 'gemini') {
          ...
          if (selector.brand === 'codex') {
            ...
            await providersApi.updateCommandCodeConfigByKey(selector.apiKey, selector.baseUrl, next);
          } else if (selector.brand === 'freebuff') {
