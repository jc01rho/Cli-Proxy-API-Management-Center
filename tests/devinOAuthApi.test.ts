...
describe('Devin Management OAuth v7.3.1 contract', () => {
  test('starts a built-in Devin flow without web UI mode', async () => {
    ...
    try {
      ...
      expect(get).toHaveBeenCalledWith('/devin-auth-url', { params: undefined });
    } finally {
      get.mockRestore();
    }
  });
  ...
  test('forwards attempt abort signals to every OAuth request', async () => {
    ...
    try {
      await oauthApi.startAuth('devin', undefined, signal);
